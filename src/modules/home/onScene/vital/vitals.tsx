...
const Vitals: FC<navigationProps> = (props) => {
  const [vitalsData, setVitalsData] = useState<Array<any>>([]);

  const addNewSet = () => {
    props.navigation.navigate('AddVitals');
  };
  const viewSet = (vitals) => {
    props.navigation.navigate('ViewVitals', {vitalsObj: vitals});
  };
  const assistProfileClicked = () => {};
  const assistProfileChanged = (assistObj) => {
    setVitalsData(props.vitals.getVitalsByID(assistObj.assistUserID));
  };
  return (
...
export default inject('vitals')(observer(Vitals));
